...
public class TopTrackService {
    ...
    /**
     * Get the user's top tracks from Spotify API.
     *
     * @param token The access token required for authentication with the Spotify API.
     * @param term  The time range for which to get the top tracks (0 for short-term, 1 for medium-term, 2 for long-term).
     * @return The user's top tracks as a LinkedHashMap containing the response from the Spotify API.
     * @throws NoAccountDataException if there are no top tracks for the given time range.
     */
    public LinkedHashMap<String, Object> getTopTracks(String token, int term) throws NoAccountDataException {
        ...
        // Retrieve the "items" list containing top tracks
        List<?> items = (List<?>) result.get("items");

        // Check if there are any top tracks in the list
